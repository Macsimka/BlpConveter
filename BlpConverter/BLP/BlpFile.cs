...
namespace BlpConverter.BLP
{
    ...
    public struct ARGBColor8
    {
        ...
        public static void ConvertToBGRA(byte[] pixel)
        {
            for (int i = 0; i < pixel.Length; i += 4)
                (pixel[i], pixel[i + 2]) = (pixel[i + 2], pixel[i]);
        }
    }
    ...
    public sealed class BlpFile : IDisposable
    {
        ...
        // Public properties for metadata access
        public int Width => width;
        public int Height => height;
        public BlpColorEncoding ColorEncoding => colorEncoding;
        public BlpPixelFormat PixelFormat => preferredFormat;
        public int AlphaDepth => alphaSize;
        public bool HasMipmaps => hasMips == 1;
